...
function roundObjectValues(obj) {
  ...
  const convertedObject = Object.fromEntries(roundedEntries);
...
function getFractionalPart(num) {
  ...
  const floatFractionalPart = Math.fround(fractionalPart);
...
function compareAndRound(num1, num2) {
  ...
  const roundedNum = Math.round(largestNum);
...
function estimateInvestment(principal, interestRate, years) {
  ...
  const sumInvest = principal * Math.pow(1 + interestRate, years);

  const roundedSumInvest = Math.round(sumInvest);

  return roundedSumInvest;
}
